...
func runWritingTurns(ctx context.Context, execution *agentcore.Execution, handler func(context.Context, int) (RunResult, bool, error)) (RunResult, error) {
	var result RunResult
	err := execution.RunTurns(ctx, func(turnCtx context.Context, step int) (bool, error) {
		turnResult, done, turnErr := handler(turnCtx, step)
		if done {
			result = turnResult
		}
		return done, turnErr
...
func NewLoop(skills SkillCatalog, tools *agentcore.ToolRegistry, budget agentcore.Budget) *Loop {
	return &Loop{skills: skills, tools: tools, runtime: agentcore.NewRuntime(tools, budget)}
}

func (l *Loop) Run(ctx context.Context, input RunInput, model TextModel, emit Emitter) (RunResult, error) {
	if l == nil || l.skills == nil || l.tools == nil || l.runtime == nil {
		return RunResult{}, errors.New("agent loop dependencies are not configured")
	}
	var result RunResult
	err := l.runtime.Run(ctx, model, emit, func(runCtx context.Context, execution *agentcore.Execution) error {
		var runErr error
		result, runErr = l.runWorkflow(runCtx, execution, input, emit)
		return runErr
	})
	return result, err
}

func (l *Loop) runWorkflow(runCtx context.Context, execution *agentcore.Execution, input RunInput, emit Emitter) (RunResult, error) {
	...
	if IsCollaborativeTarget(input.Target) {
		return l.runCollaborative(runCtx, execution, input, emit)
	}
	...
	return runWritingTurns(runCtx, execution, func(runCtx context.Context, step int) (RunResult, bool, error) {
		decision, _, err := requestDecision(
			runCtx,
			model,
			input.ModelID,
			state.decisionPrompt(step, l.tools),
			execution.RemainingModelCalls(),
			emit,
		)
		if err != nil {
		...
		case DecisionCallTool:
			if state.activeSkill.ID == "" {
				return RunResult{}, false, errors.New("model requested a tool before selecting a skill")
			}
			result, err := execution.CallTool(runCtx, decision.ToolName, agentcore.ToolInvocation{
		...
		case DecisionProduceCandidate:
			if state.activeSkill.ID == "" {
				return RunResult{}, false, errors.New("model requested candidate production before selecting a skill")
			}
			result, err := l.produceCandidate(runCtx, execution, state, model, emit)
			if err != nil {
				return RunResult{}, false, err
			}
		...
		case DecisionFinish:
			if state.activeSkill.ID == "" {
				return RunResult{}, false, errors.New("model finished before selecting a skill")
			}
			if strings.TrimSpace(decision.Content) == "" {
				return RunResult{}, false, errors.New("finish decision requires content")
			}
			result, err := l.completeContent(runCtx, execution, state, strings.TrimSpace(decision.Content), emit)
			return result, err == nil, err
		case DecisionFail:
			return RunResult{}, false, fmt.Errorf("agent stopped: %s", decision.Reason)
		default:
			return RunResult{}, false, fmt.Errorf("unsupported decision kind %q", decision.Kind)
		}
		return RunResult{}, false, nil
	})
}
...
func (l *Loop) produceCandidate(ctx context.Context, execution *agentcore.Execution, state loopState, model TextModel, emit Emitter) (RunResult, error) {
	if IsNodeUpdateTarget(state.input.Target) {
		return l.produceNodeUpdate(ctx, execution, state, model, emit)
	}
	if state.input.Target == TargetSectionOutlineBatch || state.input.Target == TargetChapterSection || state.input.Target == TargetChapterArchive {
		return l.produceDerivation(ctx, execution, state, model, emit)
	}
	...
	return l.completeContent(ctx, execution, state, result, emit)
}

func (l *Loop) produceDerivation(ctx context.Context, execution *agentcore.Execution, state loopState, model TextModel, emit Emitter) (RunResult, error) {
...
func (l *Loop) produceNodeUpdate(ctx context.Context, execution *agentcore.Execution, state loopState, model TextModel, emit Emitter) (RunResult, error) {
...
func (l *Loop) completeContent(ctx context.Context, execution *agentcore.Execution, state loopState, content string, emit Emitter) (RunResult, error) {
	...
	created, err := execution.CallTool(ctx, "canvas.create_candidate", agentcore.ToolInvocation{
		RunID: state.input.RunID, WorkID: state.input.WorkID,
		SkillID: state.activeSkill.ID, SkillVersion: state.activeSkill.Version,
		AllowedTools: state.activeSkill.AllowedTools, Args: toolArgs,
	}, nil)
	if err != nil {
...
func (s loopState) decisionPrompt(step int, tools *agentcore.ToolRegistry) string {
