...
import (
	...
	"warmnote/core/internal/agent/guardrail"
...
func NewWorkFileRepository(dataDirectory string) *WorkFileRepository {
	return &WorkFileRepository{dataDirectory: dataDirectory}
}

func (r *WorkFileRepository) SearchText(ctx context.Context, workID, scope, query string, limit int) ([]workspace.SearchResult, error) {
	boundary, err := guardrail.ResolveWorkSearchBoundary(r.dataDirectory, workID, scope)
	if err != nil {
		return nil, err
	}
...
func searchTextWithRG(ctx context.Context, rgPath, workRoot, searchRoot, fileGlob, query string, limit int) ([]workspace.SearchResult, error) {
...
func searchTextWithGo(workRoot, searchRoot, fileGlob, query string, limit int) ([]workspace.SearchResult, error) {
...
func readSearchResults(workRoot string, paths []string, query string, limit int, source string) ([]workspace.SearchResult, error) {
	...
	results := make([]workspace.SearchResult, 0, min(limit, len(files)))
