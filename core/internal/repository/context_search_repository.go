...
const (
	ContextEmbeddingDimension = model.CanonicalEmbeddingDimensions
	contextIndexBatchSize     = 8
	contextSearchOversample   = 4
)

// ContextEmbedder is deliberately independent from the chat model. ModelID
...
func (g *ContextSearchGateway) SearchContext(ctx context.Context, workID string, input canvas.ContextSearchInput) ([]canvas.ContextSearchResult, error) {
...
func (i *ContextIndex) SearchContext(ctx context.Context, workID string, input canvas.ContextSearchInput) ([]canvas.ContextSearchResult, error) {
	if err := i.configured(); err != nil {
		return nil, err
	}
...
WHERE v.embedding MATCH ? AND k=? AND v.work_id=? AND v.model_id=?
  AND d.work_id=? AND d.status='ready' AND d.model_id=?`
	...
	results := make([]canvas.ContextSearchResult, 0, input.Limit)
	seen := make(map[string]int)
	for rows.Next() {
		var result canvas.ContextSearchResult
	...
	var graphResults []canvas.ContextSearchResult
...
func contextSearchResultKey(result canvas.ContextSearchResult) string {
	return strings.Join([]string{result.NodeID, result.VersionID, result.ArchiveID, result.Scope}, "|")
}

func (i *ContextIndex) searchRelatedNodes(ctx context.Context, workID string, input canvas.ContextSearchInput) ([]canvas.ContextSearchResult, error) {
