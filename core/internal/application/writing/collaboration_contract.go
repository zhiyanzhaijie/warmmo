...
func ValidateProposalSet(value string) error {
	_, err := parseProposalSet(value)
	return err
}

func ValidateProposalSetForSkill(value, skillID string) error {
	proposal, err := parseProposalSet(value)
	if err != nil {
		return err
	}
	if len(proposal.Updates) > 0 || len(proposal.BaseRevisions) > 0 {
		return fmt.Errorf("skill %q cannot update existing nodes", skillID)
	}
	switch skillID {
	case "entity-creator":
		for index, node := range proposal.Nodes {
			kind, _ := canvas.ParseNodeKind(node.Kind)
			if !containsNodeKind(canvas.EntityNodeKinds(), kind) {
				return fmt.Errorf("entity creator proposal node %d has unsupported kind %q", index, node.Kind)
			}
		}
	case "chapter-creator":
		if len(proposal.Nodes) != 1 || proposal.Nodes[0].Kind != string(canvas.NodeKindChapterOutline) {
			return errors.New("chapter creator must propose exactly one chapter-outline node")
		}
	default:
		return fmt.Errorf("skill %q cannot produce proposal output", skillID)
	}
	return nil
}

func parseProposalSet(value string) (ProposalSet, error) {
	if !json.Valid([]byte(value)) {
		return ProposalSet{}, errors.New("proposal set must be valid JSON")
	}
	var fields map[string]json.RawMessage
	if err := json.Unmarshal([]byte(value), &fields); err != nil || fields == nil {
		return ProposalSet{}, errors.New("proposal set must be a JSON object")
	}
	for _, field := range []string{"baseRevisions", "nodes", "updates", "edges", "reasons", "questions"} {
		if _, exists := fields[field]; !exists {
			return ProposalSet{}, fmt.Errorf("proposal set requires top-level field %q", field)
		}
	}
	...
	if err := decoder.Decode(&proposal); err != nil {
		return ProposalSet{}, fmt.Errorf("decode proposal set: %w", err)
	}
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return ProposalSet{}, errors.New("proposal set must contain exactly one JSON object")
	}
	if len(proposal.Nodes) == 0 && len(proposal.Updates) == 0 {
		return ProposalSet{}, errors.New("proposal set must contain at least one node or update")
	}
	if len(proposal.Nodes) > MaxProposalNodes {
		return ProposalSet{}, fmt.Errorf("proposal set must contain at most %d nodes", MaxProposalNodes)
	}
	...
	for index, node := range proposal.Nodes {
		...
		if clientID == "" ||
			...
			return ProposalSet{}, fmt.Errorf("proposal node %d requires clientId, kind, title, and content", index)
		}
		if !validKind || !canvas.IsManuallyCreatableNodeKind(kind) {
			return ProposalSet{}, fmt.Errorf("proposal node %d has unsupported creatable kind %q", index, node.Kind)
		}
		if _, exists := clientIDs[clientID]; exists {
			return ProposalSet{}, fmt.Errorf("proposal node %d repeats clientId %q", index, clientID)
		}
	...
	for index, update := range proposal.Updates {
		if strings.TrimSpace(update.NodeID) == "" || update.BaseRevision < 1 ||
			...
			return ProposalSet{}, fmt.Errorf("proposal update %d requires nodeId, positive baseRevision, title, and content", index)
		}
	}
	for index, edge := range proposal.Edges {
		if strings.TrimSpace(edge.SourceID) == "" || strings.TrimSpace(edge.TargetID) == "" || strings.TrimSpace(edge.Kind) == "" {
			return ProposalSet{}, fmt.Errorf("proposal edge %d requires sourceId, targetId, and kind", index)
		}
		if edge.SourceID == edge.TargetID {
			return ProposalSet{}, fmt.Errorf("proposal edge %d cannot connect an endpoint to itself", index)
		}
		if edge.Kind != "generated_from" {
			return ProposalSet{}, fmt.Errorf("proposal edge %d has unsupported kind %q", index, edge.Kind)
		}
	}
	return proposal, nil
}

func containsNodeKind(kinds []canvas.NodeKind, target canvas.NodeKind) bool {
