...
func (t *SearchTextTool) Spec() appharness.ToolSpec {
	return appharness.ToolSpec{
		...
		SideEffect:  appharness.SideEffectRead, Approval: appharness.ApprovalNever, MaxResultBytes: 32 * 1024,
...
func (t *SearchTextTool) Call(ctx context.Context, invocation appharness.ToolInvocation) (any, error) {
	var input struct {
	...
	if input.Scope == "" {
		return nil, appharness.NewToolError(appharness.ToolErrorInvalidArgument, false, fmt.Errorf("scope is required"))
	}
	if input.Scope != workspace.ScopeStorySpine {
		return nil, appharness.NewToolError(
			appharness.ToolErrorInvalidArgument, false, fmt.Errorf("scope must be %q", workspace.ScopeStorySpine),
		)
	}
	if input.Limit == 0 {
	...
	if input.Limit < 1 || input.Limit > workspace.MaxLimit {
		return nil, appharness.NewToolError(appharness.ToolErrorInvalidArgument, false, fmt.Errorf("limit must be between 1 and %d", workspace.MaxLimit))
	}
...
func decodeSearchArgs(data json.RawMessage, target any) error {
	...
	if err := decoder.Decode(target); err != nil {
		return appharness.NewToolError(appharness.ToolErrorInvalidArgument, false, fmt.Errorf("decode workspace search arguments: %w", err))
	}
	return nil
}
