...
func (t *SearchStorySpineTool) Spec() appharness.ToolSpec {
...
func (t *SearchStorySpineTool) Call(ctx context.Context, invocation appharness.ToolInvocation) (any, error) {
	var input struct {
	...
	if input.Limit < 1 || input.Limit > maxStorySpineSearchLimit {
		return nil, appharness.NewToolError(appharness.ToolErrorInvalidArgument, false, fmt.Errorf("limit must be between 1 and %d", maxStorySpineSearchLimit))
	}
	...
	results, err := t.database.SearchStorySpineDatabase(ctx, invocation.WorkID, input.Query, input.Limit)
	return results, canvasToolError(err)
}
