...
const (
	// StructuredOutputPrompt works with providers that do not implement an
	// OpenAI response_format extension. It is the conservative default.
...
const structuredOutputToolName = "set_model_response"

type StructuredOutputRegistry map[string]StructuredOutputMode

func (r StructuredOutputRegistry) Resolve(providerID string) StructuredOutputMode {
	if mode, ok := r[providerID]; ok {
		return mode
	}
...
const MalformedArgumentsKey = "__warmmo_malformed_arguments"

var ErrInvalidStructuredOutput = errors.New("invalid structured model output")
...
func NewOpenAICompatible(config OpenAICompatibleConfig, client *http.Client) adkmodel.LLM {
	if client == nil {
...
func (m *compatibleLLM) GenerateContent(ctx context.Context, request *adkmodel.LLMRequest, stream bool) iter.Seq2[*adkmodel.LLMResponse, error] {
	return func(yield func(*adkmodel.LLMResponse, error) bool) {
		chatRequest, err := buildChatRequest(request, m.config.ModelID, m.config.StructuredOutput, stream)
		if err != nil {
			yield(nil, err)
			return
		}
		...
		if stream {
			streamChatResponse(response.Body, yield, chatRequest.codec, chatRequest.structuredOutput)
			return
		}
		completeChatResponse(response.Body, yield, chatRequest.codec, chatRequest.structuredOutput)
	}
}
...
func buildChatRequest(
	...
	if config.ResponseMIMEType == "application/json" {
		...
		case StructuredOutputJSONSchema:
			...
			result.ResponseFormat = &chatResponseFormat{
				Type:       "json_schema",
				JSONSchema: &chatJSONSchema{Name: "agent_response", Strict: true, Schema: config.ResponseSchema},
			}
		case StructuredOutputJSONObject:
			result.ResponseFormat = &chatResponseFormat{Type: "json_object"}
			if config.ResponseSchema != nil {
				result.Messages, err = appendStructuredOutputInstruction(result.Messages, config.ResponseSchema)
				if err != nil {
					return chatRequest{}, err
				}
			}
		case StructuredOutputToolCall:
			if config.ResponseSchema != nil {
				if _, exists := codec.internalToExternal[structuredOutputToolName]; exists {
					return chatRequest{}, fmt.Errorf("structured output tool name %q conflicts with a registered tool", structuredOutputToolName)
				}
				result.structuredOutput = newStructuredOutputTool(config.ResponseSchema)
				result.Messages = appendInstruction(result.Messages,
					"After using any other tools needed, call set_model_response with the final answer. Do not return the final answer as text.")
			}
		...
		default:
			return chatRequest{}, fmt.Errorf("unsupported structured output mode %q", structuredOutput)
		}
	}
	...
	if result.structuredOutput != nil {
		result.Tools = append(result.Tools, result.structuredOutput.tool)
	}
...
func appendStructuredOutputInstruction(messages []chatMessage, schema *genai.Schema) ([]chatMessage, error) {
	encoded, err := json.Marshal(schema)
	if err != nil {
		return nil, fmt.Errorf("encode response schema instruction: %w", err)
	}
	instruction := "Return only valid JSON matching this JSON Schema. Do not include markdown fences or explanatory text:\n" + string(encoded)
	if schema.Type == genai.TypeString {
		instruction = "Return only the string value matching this schema as plain text. Do not add JSON quotes, markdown fences, or explanatory text:\n" + string(encoded)
	}
	return appendInstruction(messages, instruction), nil
}

func appendInstruction(messages []chatMessage, instruction string) []chatMessage {
	if len(messages) > 0 && messages[0].Role == "system" && messages[0].Content != nil {
		value := strings.TrimSpace(*messages[0].Content) + "\n\n" + instruction
		messages[0].Content = &value
		return messages
	}
	return append([]chatMessage{newTextMessage("system", instruction)}, messages...)
}
...
func newStructuredOutputTool(schema *genai.Schema) *structuredOutputTool {
	parameters := any(schema)
	unwrap := schema.Type != genai.TypeObject
	if unwrap {
		parameters = map[string]any{
			"type": "object", "properties": map[string]any{"value": schema},
			"required": []string{"value"}, "additionalProperties": false,
		}
	}
	return &structuredOutputTool{tool: chatTool{Type: "function", Function: chatFunctionDefinition{
		Name:        structuredOutputToolName,
		Description: "Set the final response using the required output schema.",
		Parameters:  parameters,
	}}, unwrap: unwrap}
}

func (t *structuredOutputTool) content(calls []chatToolCall) (*genai.Content, bool, error) {
	if t == nil {
		return nil, false, nil
	}
	var arguments string
	for _, call := range calls {
		if call.Function.Name == structuredOutputToolName {
			if arguments != "" {
				return nil, true, errors.New("model returned the structured output tool more than once")
			}
			arguments = call.Function.Arguments
			continue
		}
		if arguments != "" || hasStructuredOutputCall(calls) {
			return nil, true, errors.New("model mixed the structured output tool with executable tool calls")
		}
	}
	if arguments == "" {
		return nil, false, nil
	...
	if err := json.Unmarshal([]byte(arguments), &value); err != nil {
		return nil, true, fmt.Errorf("%w: decode tool arguments: %v", ErrInvalidStructuredOutput, err)
	}
	if t.unwrap {
		object, ok := value.(map[string]any)
		if !ok {
			return nil, true, errors.New("structured output tool arguments must be an object")
		}
		var exists bool
		value, exists = object["value"]
		if !exists || len(object) != 1 {
			return nil, true, errors.New("structured output tool requires exactly one value field")
		}
	}
	encoded, err := json.Marshal(value)
	if err != nil {
		return nil, true, fmt.Errorf("encode structured output tool result: %w", err)
	}
	return genai.NewContentFromText(string(encoded), genai.RoleModel), true, nil
}

func hasStructuredOutputCall(calls []chatToolCall) bool {
	for _, call := range calls {
		if call.Function.Name == structuredOutputToolName {
			return true
		}
...
func streamChatResponse(
	body io.Reader,
	yield func(*adkmodel.LLMResponse, error) bool,
	codec toolNameCodec,
	structuredOutput *structuredOutputTool,
) {
	accumulator := newStreamAccumulator(codec, structuredOutput)
	completed := false
...
func completeChatResponse(
	...
	content, err := completionContent(choice.Message, codec, structuredOutput)
	if err != nil {
		yield(nil, err)
		return
	}
...
func completionContent(
	...
	if structuredOutput != nil {
		if content, found, err := structuredOutput.content(message.ToolCalls); found || err != nil {
			return content, err
		}
	}
...
func newStreamAccumulator(codec toolNameCodec, structuredOutput *structuredOutputTool) *streamAccumulator {
	return &streamAccumulator{
		toolCalls: make(map[int]*chatToolCall),
		codec:     codec, structuredOutput: structuredOutput,
	}
}
...
func (a *streamAccumulator) finalResponse() (*adkmodel.LLMResponse, error) {
	...
	toolCalls := make([]chatToolCall, 0, len(a.toolOrder))
	for _, index := range a.toolOrder {
		toolCalls = append(toolCalls, *a.toolCalls[index])
	}
	if a.structuredOutput != nil {
		if content, found, err := a.structuredOutput.content(toolCalls); found || err != nil {
			if err != nil {
				return nil, err
			}
			...
			return &adkmodel.LLMResponse{
				Content: content, UsageMetadata: usage,
				FinishReason: finishReason(a.finishReason), TurnComplete: true,
			}, nil
		}
	}
...
func (c chatToolCall) functionCall(codecs ...toolNameCodec) (*genai.FunctionCall, error) {
	...
	if err := json.Unmarshal([]byte(arguments), &args); err != nil {
		args = map[string]any{MalformedArgumentsKey: fmt.Sprintf("arguments must be one valid JSON object: %v", err)}
	}
	if args == nil {
...
type chatRequest struct {
	...
	structuredOutput  *structuredOutputTool `json:"-"`
}
