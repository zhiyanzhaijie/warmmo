...
export const CompactFlowNodeRenderer = memo(function CompactFlowNodeRenderer({ data, selected }: NodeProps<StoryFlowNode>) {
  ...
  return (
    <article className="group relative w-52 overflow-visible">
      ...
      <div
        className="relative flex h-12 items-center gap-space-sm overflow-hidden rounded-sm border bg-canvas-elevated px-space-sm"
        style={{
      ...
      >
        ...
        {data.sourceType === 'node' && !contentLocked ? <NodeAgentCompactIndicator nodeId={data.sourceId} /> : null}
        <span
          aria-hidden="true"
      ...
      <FlowNodeControls allowIncoming={!contentLocked} allowOutgoing={canCreateFromContext} />
    </article>
  )
})
...
export const MarkerFlowNodeRenderer = memo(function MarkerFlowNodeRenderer({ data, selected }: NodeProps<StoryFlowNode>) {
  ...
  return (
    ...
    >
      <span
        className={`relative block size-full overflow-hidden rounded-full border-2 border-canvas ${data.archiveLocked ? '' : definition.accentClassName}`}
        style={{ backgroundColor: data.archiveLocked ? accentColor : undefined }}
      >
        <span
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 rounded-[inherit]"
      ...
      {data.sourceType === 'node' && !contentLocked ? <NodeAgentMarkerIndicator nodeId={data.sourceId} /> : null}
      <FlowNodeControls allowIncoming={!contentLocked} allowOutgoing={canCreateFromContext} />
    </div>
  )
})
