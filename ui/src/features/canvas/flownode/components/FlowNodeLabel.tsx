...
export const FlowNodeLabel = memo(function FlowNodeLabel({
  ...
  return (
    <div
      className="pointer-events-none absolute bottom-full left-0 mb-space-xxs flex h-6 min-w-0 max-w-full items-center gap-space-xs select-none transition-colors"
      style={{ color: selected ? definition.accentColor : 'var(--color-mute)' }}
    >
      <Icon aria-hidden="true" className="shrink-0" size={14} />
      <span className="min-w-0 truncate text-body-sm">{data.title}</span>
      <span className="shrink-0 font-mono text-[0.625rem]">{definition.label}</span>
    </div>
  )
})
