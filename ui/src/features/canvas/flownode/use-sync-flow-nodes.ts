...
import { toFlowEdges, toFlowNodes } from '@/features/canvas/flownode/adapters'
...
export function useSyncFlowNodes(
  ...
  const flowNodes = useMemo(() => toFlowNodes(nodes ?? [], candidates ?? []), [candidates, nodes])
  const flowEdges = useMemo(() => toFlowEdges(edges ?? [], candidates ?? []), [candidates, edges])
